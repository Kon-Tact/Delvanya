...
fetch('data/questions_bank.json')
    ...
    .then(data => {
        ...
        nextButton.addEventListener('click', () => {
            ...
            dons[selectedOption.value] += points;
            console.log(points);
            console.log(dons);
            ...
            if (!buttonDesactived) {
                ...
                resultButton.addEventListener('click', () => {
                    ...
                    //Méthode de tri pour obtenir la réponse du quizz                    
                    let maxPoints = 0;
                    let maxDon = '';

                    for (let don in dons) {
                        if (dons[don] > maxPoints) {
                            maxPoints = dons[don];
                            maxDon = don;
                        }
                    }

                    //Création de l'url en y incluant le résultat du quizz
                    const url = `result_quizz_lieux.html?maxDon=${encodeURIComponent(maxDon)}`;
                    //Ouverture de la page résultat avec l'url créée
                    window.location.href = url;


                });

                //Ajoute le bouton "Calcul du résultat" au container du bouton
